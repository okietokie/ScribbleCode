...
import { IProgress, ProgressDocument, ProgressModelType } from './progress.types.js';
...
const progressSchema = new mongoose.Schema<IProgress>(
  ...
  },
  {
    ...
    toJSON: {
      transform: (doc, ret) => {
        const { __v, _id, ...rest } = ret as any;
        return {
          ...rest,
          id: (_id as any).toString(),
          userId: (ret.userId as any)._id 
            ? (ret.userId as any)._id.toString() 
            : ret.userId.toString(),
        };
      },
    },
...
progressSchema.statics.findByUserId = async function (userId: string): Promise<IProgress | null> {
  return this.findOne({ userId });
};

// Static method to upsert progress for a user
progressSchema.statics.upsertByUserId = async function (userId: string): Promise<IProgress> {
...
progressSchema.methods.toPublicJson = function (): any {
...
export const Progress = mongoose.model<IProgress, ProgressModelType>('Progress', progressSchema);
